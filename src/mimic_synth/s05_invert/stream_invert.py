...
_PROJECT_ROOT = _Path(__file__).resolve().parent.parent.parent.parent  # repo root
...
from .invert import _load_surrogate
...
def _detect_pitch_pyworld(
    ...
    sr: int,
    hop_ms: float = 5.0,
    f0_floor: float = 200.0,    # 200Hz minimum; raise for high-pitched sources
...
def _write_pitch_bend_midi(
    ...
    Args:
        ...
        pb_range_st: total pitch bend range in semitones. ±3st covers most
            within-region pitch drops in typical bird-call targets.
...
def _compute_fine_pitch_trajectory(
    ...
    for start in range(0, len(audio) - win_n + 1, hop_n):
        ...
        # still detects the pitch in low-amplitude voiced frames.
...
def _write_settings_md(
    ...
    from mimic_synth.s06_refine.audio_compare import (
...
def stream_invert(
    ...
    from mimic_synth.s06_refine.mono_utils import ensure_mono
    ...
    from mimic_synth.s03_embed.embed import Embedder
...
def _render_stream(
    ...
    if fine_traj_path.exists():
        ...
        if fine_frames:
            ...
            pb_range_st = 6.0     # ±6 semitones covers most natural pitch glides
    ...
    if not midi_generated:
        ...
        from mimic_synth.s06_refine.audio_compare import amp_release_sec as _arc
...
def _refine_loop(
    ...
    from mimic_synth.s06_refine.audio_compare import compute_ap_features, compute_sp_features, score_audio_composite, _lufs_normalize
...
def _hill_climb_step(
    ...
    """
    from mimic_synth.s06_refine.vst_hill_climb import hill_climb
    from mimic_synth.s06_refine.audio_compare import compute_ap_features, compute_sp_features, _lufs_normalize
...
def _cmaes_step(
    ...
    """
    from mimic_synth.s06_refine.vst_cmaes import cmaes_refine
    from mimic_synth.s06_refine.target_analysis import analyze_target, suggest_x0, print_analysis
    ...
    from mimic_synth.s06_refine.audio_compare import render_trajectory as _render_trajectory
    from mimic_synth.s06_refine.vst_cmaes import OSC_CONFIGS as _OSC_CONFIGS
